...
def get_company_info(company_name):
    try:
        ...
        logger.error(f"Erro ao obter informações da empresa {company_name}: {str(e)}")
        raise
...
def send_data_to_api(data, company_name, code_cvm):
    ...
    data_dict = convert_df_to_dict(data)
    dates = sorted(data_dict.keys())
    
    logging.info(f"Enviando dados para as datas: {dates}")
    
    api_url = 'http://api:8000/insert-data'
    
    for date in dates:
        data_to_send = data_dict[date]
        if not data_to_send:
            logger.warning(f"Não há dados para a data {date}")
            continue
            
        logger.info(f"Enviando dados de {company_name} para a data {date}")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.status_code == 200:
                    logger.info(f"Dados enviados com sucesso para a data {date}")
                    break
                else:
                    logger.warning(f"Tentativa {attempt+1}/{max_retries}: Status code {response.status_code}")
                    if attempt < max_retries - 1:
...
empresa = Variable.get("b3_empresa", default_var="ITSA")
ano = int(Variable.get("b3_ano", default_var="2023"))
...
t2 = PythonOperator(
    ...
    op_kwargs={'code_cvm': "{{ task_instance.xcom_pull(task_ids='get_company_info') }}", 
               'year': ano},
    provide_context=True,
    dag=dag,
)
...
t7 = PythonOperator(
    ...
    op_kwargs={
        'data': t6.output,
        'company_name': df_companies[df_companies["issuingCompany"] == empresa].iloc[0]['tradingName'],
        'code_cvm': "{{ task_instance.xcom_pull(task_ids='get_company_info') }}"
    },
    provide_context=True,
    dag=dag,
)
